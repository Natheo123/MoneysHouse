...
import {
  getDiscordPublishMapServer,
  requestAllDiscordPublishServer,
  requestDiscordPublishServer,
} from "@/lib/discord-app-publish-store";
import {
...
async function catalogAdminPayload() {
  const [apps, hiddenIds, discordPublish] = await Promise.all([
    getCustomAppsServer(),
    getHiddenAppIdsServer(),
    getDiscordPublishMapServer(),
  ]);
  return { apps, hiddenIds, discordPublish };
}
...
export async function POST(request: NextRequest) {
  ...
  if (body.action === "request-discord-all") {
    const result = await requestAllDiscordPublishServer();
    if (!result.ok) return NextResponse.json(result, { status: 400 });
    const payload = await catalogAdminPayload();
    return NextResponse.json({ ok: true, queuedCount: result.queuedCount, ...payload });
  }
