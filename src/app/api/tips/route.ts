...
import { getTipsServer, updateTipsServer } from "@/lib/tips-store";
...
export async function GET() {
  const settings = await getTipsServer();
  return NextResponse.json({ settings });
}

export async function POST(request: NextRequest) {
  let body: { settings?: unknown; requestedBy?: string };
  try {
  ...
  const result = await updateTipsServer(body.settings);
